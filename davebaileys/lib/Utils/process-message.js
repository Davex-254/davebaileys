    ...
    if (protocolMsg) {
        switch (protocolMsg.type) {
            ...
            case WAProto_1.proto.Message.ProtocolMessage.Type.REVOKE:
                ...
                // Anti-delete: Emit special event for deleted status messages
                const isStatusMessage = (0, WABinary_1.isJidStatusBroadcast)(message.key.remoteJid);
                if (isStatusMessage) {
                    ev.emit('status.delete', {
                        key: revokeKey,
                        participant: message.key.participant,
                        timestamp: message.messageTimestamp,
                        deletedBy: message.key.fromMe ? 'self' : message.key.participant
                    });
                }
                // Also emit for regular message deletes (anti-delete for all messages)
