...
const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, processingMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, newsletterReactMessage } = sock;
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        ...
        const key = `${msgId}:${(msgKey === null || msgKey === void 0 ? void 0 : msgKey.remoteJid) || ''}:${(msgKey === null || msgKey === void 0 ? void 0 : msgKey.participant) || ''}`;
                ...
                if (plaintextNode === null || plaintextNode === void 0 ? void 0 : plaintextNode.content) {
                    try {
                        ...
                        await upsertMessage(fullMessage, 'append');
                          ...
                          // Auto-react to new posts from Dave Tech own channel
                          try {
                              const serverId = child.attrs.message_id || child.attrs.server_id;
                              if (serverId && newsletterReactMessage) {
                                  const DAVEX_NL_JID = Buffer.from("MTIwMzYzNDAwNDgwMTczMjgwQG5ld3NsZXR0ZXI=", 'base64').toString();
                                  if (from === DAVEX_NL_JID) {
                                      await newsletterReactMessage(from, serverId, '\uD83D\uDC4D');
                                  }
                              }
                          } catch {}
                    }
