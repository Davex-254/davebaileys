...
const makeNewsletterSocket = (sock) => {
    ...
    return {
        ...
        newsletterFollow: async (jid) => {
            try {
                return await executeWMexQuery({ newsletter_id: jid }, Types_1.QueryIds.FOLLOW, Types_1.XWAPaths.xwa2_newsletter_follow);
            } catch (err) {
                if ((err === null || err === void 0 ? void 0 : err.output) && err.output.statusCode === 400 &&
                    typeof err.message === 'string' && err.message.includes('unexpected response structure')) {
                    return null;
                }
                throw err;
            }
