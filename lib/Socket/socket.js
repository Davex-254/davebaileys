...
const makeSocket = (config) => {
    ...
    /** Update our clock drift estimate relative to WhatsApp's server time */
    const updateServerTimeOffset = (node) => {
        var _a;
        const tValue = (_a = node === null || node === void 0 ? void 0 : node.attrs) === null || _a === void 0 ? void 0 : _a.t;
        if (!tValue) return;
    ...
    /** Returns a stable session ID derived from server-synced time — used to anchor the connection */
    const getUnifiedSessionId = () => {
        const WEEK_MS = 7 * 24 * 60 * 60 * 1000;
        const DAY_3_MS = 3 * 24 * 60 * 60 * 1000;
        const now = Date.now() + serverTimeOffsetMs;
        return ((now + DAY_3_MS) % WEEK_MS).toString();
    };
    /**
     * Sends a `unified_session` anchor to WhatsApp after login.
     * This is the primary fix for the 428 reconnection loop on deployed servers:
     * without it WhatsApp does not have a stable session identifier and may close
     * the connection with 428 (connectionClosed) immediately after every reconnect.
     */
    const sendUnifiedSession = async () => {
        ...
        try {
            ...
            logger.debug({ error }, 'failed to send unified_session');
        }
    };
    /**
     * Validates the current key bundle against the WhatsApp server.
     * If the server reports the bundle is missing, uploads fresh pre-keys.
     */
    const digestKeyBundle = async () => {
        try {
            ...
            logger.warn({ err }, 'digestKeyBundle failed, skipping');
        }
    };
