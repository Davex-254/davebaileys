...
export const makeSocket = (config) => {
    ...
    /** Sync clock drift from the server timestamp in any incoming node */
    const updateServerTimeOffset = (node) => {
        const tValue = node?.attrs?.t;
        if (!tValue) return;
        const parsed = Number(tValue);
        if (Number.isNaN(parsed) || parsed <= 0) return;
        serverTimeOffsetMs = parsed * 1000 - Date.now();
        logger.debug({ offset: serverTimeOffsetMs }, 'updated server time offset');
    };
    /**
     * Sends a unified_session anchor to WhatsApp after login.
     * Fixes the 428 reconnection loop: without it, WhatsApp has no stable
     * session ID across IP changes and closes the connection immediately.
     */
    const sendUnifiedSession = async () => {
        if (!ws.isOpen) return;
        const WEEK_MS = 7 * 24 * 60 * 60 * 1000;
        const DAY_3_MS = 3 * 24 * 60 * 60 * 1000;
        const sessionId = ((Date.now() + serverTimeOffsetMs + DAY_3_MS) % WEEK_MS).toString();
        try {
            await sendNode({
                tag: 'ib',
                attrs: {},
                content: [{ tag: 'unified_session', attrs: { id: sessionId } }]
            });
        } catch (err) {
            logger.debug({ err }, 'failed to send unified_session');
        }
    };
