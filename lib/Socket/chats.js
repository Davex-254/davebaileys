...
const makeChatsSocket = (config) => {
    ...
    /** separate mutexes matching official Baileys — prevents cross-handler deadlocks */
    ...
    const appStatePatchMutex = processingMutex;
    ...
    const appPatch = async (patchCreate) => {
        ...
        let myAppStateKeyId = authState.creds.myAppStateKeyId;
        if (!myAppStateKeyId) {
            logger.info({ name }, 'myAppStateKeyId not set — attempting full app state resync before patch');
            try {
                await resyncAppState(Types_1.ALL_WA_PATCH_NAMES, true);
                myAppStateKeyId = authState.creds.myAppStateKeyId;
            } catch (resyncErr) {
                logger.warn({ err: resyncErr }, 'app state resync attempt failed');
            }
            if (!myAppStateKeyId) {
     ...
     * @param lastMessages - required last messages in the chat (sorted newest first)
     */
    const archiveChat = (jid, archive, lastMessages = []) => {
     ...
     * @param lastMessages - last messages in the chat
     */
    const readChat = (jid, read, lastMessages = []) => {
    ...
    /**
     * Delete a single message for yourself (not for everyone)
     * @param jid - the chat JID
     * @param message - the message to delete ({ id, fromMe, timestamp })
     */
    const deleteMessageForMe = (jid, message) => {
        return chatModify({
            deleteForMe: {
                key: message,
    ...
    /**
     * Create a WhatsApp call link (audio or video)
     * Pulled from official Baileys
     */
    ...
    /**
     * Add or edit a contact action via app state patch
     * Pulled from official Baileys
     */
    ...
    /**
     * Remove a contact via app state patch
     * Pulled from official Baileys
     */
    ...
    /**
     * Remove a quick reply by timestamp
     * Pulled from official Baileys
     */
    ...
    /**
     * Enable or disable link preview generation globally
     * Pulled from official Baileys
     */
    ...
    ev.on('lid-mapping.update', async ({ lid, pn }) => {
        try {
            await sock.signalRepository.lidMapping.storeLIDPNMappings([{ lid, pn }]);
        } catch (error) {
